...
'd3-uni':{stay:'약 1시간',next:'🚗 약 1시간 50분',nextName:'오타루 운하'},
'd3-shimamui':{stay:'약 30분',next:'🚗 약 1시간 30분',nextName:'오타루'},
...
'd1-moiwa':{minutes:80,extra:'약 +1시간 20분',detour:'도심에서 별도 왕복',why:'야경은 좋지만 첫날 피로와 다음날 06:30 출발을 생각하면 선택'},
...
'd3-shimamui':{minutes:35,extra:'약 +35분',detour:'동선 변화 거의 없음',why:'오타루 방향 귀로에 가까워 시간·체력 여유가 있을 때만 추가'},
'd3-sakaimachi':{minutes:55,extra:'약 +55분',detour:'오타루 시내라 우회 거의 없음',why:'르타오·롯카테이 쇼핑이 목적이면 추천, 늦으면 운하를 우선'},
...
function getCurrentStop(){const now=new Date(),today=todayISO();if(today<'2026-08-28')return schedule.find(s=>!isOptional(s))||schedule[0];if(today>'2026-09-01')return null;const list=schedule.filter(s=>s.date===today&&!isDone(s)&&!isOptional(s));if(!list.length)return null;return list.find(s=>dt(s)>=new Date(now.getTime()-45*60000))||list[list.length-1]}
...
document.addEventListener('DOMContentLoaded',()=>{refreshWeather();restoreDone();markToday();initOptionPlan();initTimingMeta();initSearchButtons();initNotes();applyConfirmedHotels();initMap();updateNowCard();updateProgress();onlineState();setInterval(updateNowCard,60000);if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{})});