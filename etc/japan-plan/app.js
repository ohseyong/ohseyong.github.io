...
{key:'d3-dinner',date:'2026-08-30',time:'19:00',title:'🍣 오타루 스시',desc:'19:00~20:15 저녁 · 이후 GRIDS PREMIUM Hotel Otaru로 이동',dest:'Sushi Otaru Hokkaido',lat:43.1907,lon:140.9947,nav:true},
{key:'d3-hotel',date:'2026-08-30',time:'20:30',title:'🏨 GRIDS PREMIUM Hotel Otaru 체크인',desc:'예약 확정 · 운하·사카이마치 도보 4분 · 주차 총 45대 선착순, 1박 1,500엔',dest:'GRIDS Premium Hotel Otaru, 1-3-13 Inaho, Otaru, Hokkaido 047-0032, Japan',lat:43.195777,lon:140.998928,nav:true},
...
'd3-dinner':{stay:'약 1시간 15분',next:'🚶/🚗 약 10분',nextName:'GRIDS PREMIUM Hotel Otaru'},
'd3-hotel':{stay:'숙박',next:'다음날 08:30 출발',nextName:'도야호 방향'},
...
function applyDay3Revision(){const order=['d3-depart','d3-uni','d3-kamui','d3-shimamui','d3-sakaimachi','d3-canal','d3-dinner','d3-hotel'],box=$('day3')&&$('day3').querySelector('.stops'),byKey=Object.fromEntries(schedule.map(s=>[s.key,s]));if(box&&!$('stop-d3-hotel')){const s=byKey['d3-hotel'],el=document.createElement('div');el.className='stop';el.dataset.key='d3-hotel';el.id='stop-d3-hotel';el.innerHTML=`<div class="time">${s.time}</div><div class="stop-main"><div class="stop-title"><h3>${s.title}</h3><button class="done-btn" onclick="toggleDone(this)">✓</button></div><div class="desc">${s.desc}</div></div>`;if(localStorage.getItem('hk-d3-hotel')==='1')el.classList.add('done');box.appendChild(el)}if(box)order.forEach(k=>{const el=$('stop-'+k);if(el)box.appendChild(el)});order.forEach(k=>{const el=$('stop-'+k),s=byKey[k];if(!el||!s)return;const t=el.querySelector('.time'),h=el.querySelector('.stop-title h3'),d=el.querySelector('.desc');if(t)t.textContent=s.time;if(h)h.textContent=s.title;if(d)d.textContent=s.desc});const day=$('day3');if(day){const sub=day.querySelector('.day-sub');if(sub)sub.textContent='08:00 출발 · 우니 먼저 → 신카무이곶 → 오타루 → GRIDS PREMIUM 숙박. 시마무이는 기본 생략.';const route=day.querySelector('.day-actions .pill.primary');if(route){route.href='https://www.google.com/maps/dir/?api=1&origin=Furano+Natulux+Hotel&destination=GRIDS+Premium+Hotel+Otaru&waypoints=Bikuni+Shakotan%7CCape+Kamui%7CSakaimachi+Street+Otaru%7COtaru+Canal&travelmode=driving';route.textContent='오늘 핵심 경로'}}}
function applyConfirmedHotels(){const el=$('stop-d2-checkin');if(el){const h=el.querySelector('.stop-title h3');const d=el.querySelector('.desc');if(h)h.textContent='🏨 Furano Natulux Hotel 체크인';if(d)d.textContent='예약 확정 · JR 후라노역 도보 1~2분 · 무료 주차 · 체크인 15:00 / 체크아웃 10:00.';const main=el.querySelector('.stop-main');let links=main&&main.querySelector('.map-links');if(main&&!links){links=document.createElement('div');links.className='map-links';main.appendChild(links)}if(links&&!links.querySelector('[data-hotel-map]')){const a=document.createElement('a');a.dataset.hotelMap='1';a.target='_blank';a.rel='noopener';a.href='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent('Furano Natulux Hotel, 1-35 Asahimachi, Furano, Hokkaido');a.textContent='지도';links.prepend(a)}}const hotel2=document.querySelector('[data-field="hotel2"]');const memo2=document.querySelector('[data-field="hotel2memo"]');if(hotel2&&!localStorage.getItem('hk-field-hotel2'))hotel2.value='Furano Natulux Hotel · 예약 확정';if(memo2&&!localStorage.getItem('hk-field-hotel2memo'))memo2.value='8/29 체크인 · 무료 주차 · JR 후라노역 앞 · 체크아웃 10:00';const hotel3=document.querySelector('[data-field="hotel3"]'),memo3=document.querySelector('[data-field="hotel3memo"]');if(hotel3){hotel3.value='GRIDS PREMIUM Hotel Otaru · 예약 확정';localStorage.setItem('hk-field-hotel3',hotel3.value)}if(memo3){memo3.value='8/30 체크인 · 1-3-13 Inaho · 운하/사카이마치 도보 4분 · 주차 45대 선착순 1,500엔 · 체크아웃 11:00';localStorage.setItem('hk-field-hotel3memo',memo3.value)}const b=$('day3')&&$('day3').querySelector('[data-booking-search]');if(b){b.textContent='🏨 GRIDS 예약 확정';b.href='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent('GRIDS Premium Hotel Otaru, 1-3-13 Inaho, Otaru, Hokkaido');b.title='GRIDS PREMIUM Hotel Otaru · 예약 확정'}}
